...
def build_gpmp2_cost_composite(
    ...
    **kwargs,
    ...
    for collision_field in collision_fields:
        # Accept either:
        #  - collision_field (existing behaviour)
        #  - or tuple (collision_field, time_mask) where time_mask is tensor/list of length n_support_points
        time_mask = None
        if isinstance(collision_field, tuple):
            print('Using collision field with time mask')
            field, time_mask = collision_field
        else:
        ...
        cost_collision = CostCollision(
            ...
            time_mask=time_mask,  # new optional argument
            tensor_args=tensor_args
        )
