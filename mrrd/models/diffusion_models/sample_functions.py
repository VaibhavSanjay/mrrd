...
def apply_hard_conditioning(x, conditions):
    ...
    if 'goals' in conditions:
        num_goals = len(conditions['goals'])
        interval_size = int(math.ceil(x.shape[0] / num_goals))
        for i in range(num_goals):
            goal_pos = conditions['goals'][i]
            x[i * interval_size:(i + 1) * interval_size, -1, :] = goal_pos.clone()
    return x
...
def ddpm_sample_fn(
    ...
    # print(f'p_mean_variance took {time.time() - t0:.4f}s')
    x = model_mean
    ...
    if guide is not None and t_single < t_start_guide:
        ...
        # print(f'guide_gradient_steps took {time.time() - t0:.4f}s')
