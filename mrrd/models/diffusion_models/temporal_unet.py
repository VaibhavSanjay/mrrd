...
from mrrd.models.layers.layers import GaussianFourierProjection, Downsample1d, Conv1dBlock, Upsample1d, \
    ResidualTemporalBlock, TimeEncoder, MLP, group_norm_n_groups, LinearAttention, PreNorm, Residual, TemporalBlockMLP
from mrrd.models.layers.layers_attention import SpatialTransformer
...
class TemporalUnet(nn.Module):
    ...
    def __init__(
        ...
        if conditioning_type is None or conditioning_type == 'None':
            ...
            if False: # self.state_dim < conditioning_embed_dim // 4:
...
class GoalModel(nn.Module):
    def __init__(
            self,
            in_dim=2,      # (x, y)
            out_dim=16,    # Size of the goal embedding
            **kwargs
    ):
        super().__init__()
        self.in_dim = in_dim
        ...
        # Simple MLP to encode the 2D coordinate into a latent vector
    ...
    def forward(self, input_d):
        # Expects input_d to have the key 'goal' from your new loader
        goal = input_d['goal'] 
...
class ContextModel(nn.Module):
    def __init__(
            ...
            task_model=None,
            goal_model=None,
            time_emb_dim=32, # New arg for time embedding size
            **kwargs
    ):
        super().__init__()
        ...
        if time_emb_dim:
            self.time_embedding = nn.Embedding(num_embeddings=120, embedding_dim=time_emb_dim)

        # Calculate total dimension
        self.in_dim = 0
        if self.env_model:  self.in_dim += self.env_model.out_dim
        if self.task_model: self.in_dim += self.task_model.out_dim
        if self.goal_model: self.in_dim += self.goal_model.out_dim
        if self.time_emb_dim: self.in_dim += self.time_emb_dim
    ...
    def forward(self, input_d=None):
        ...
        if self.env_model: embs.append(self.env_model(input_d))
        if self.task_model: embs.append(self.task_model(input_d))
        if self.goal_model: embs.append(self.goal_model(input_d))
        if self.time_embedding: embs.append(self.time_embedding(input_d['goal_time']).squeeze(1))

        context = torch.cat(embs, dim=-1)
