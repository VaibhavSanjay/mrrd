...
from mrrd import trainer
from mrrd.models import UNET_DIM_MULTS, TemporalUnet, GoalModel, ContextModel
from mrrd.trainer import get_dataset, get_model, get_loss, get_summary
from mrrd.trainer.trainer import get_num_epochs
...
def experiment(
    ...
    **kwargs
    ...
    # (Assuming you don't use env/task for now, we set them to None or generic identity)
    # If you ARE using them, keep them initialized as before.

    context_model = ContextModel(
        env_model=None, 
        task_model=None,
        goal_model=goal_model  # Pass the new model
    )

    # --- 2. Calculate Total Dimension ---
    # If using only goal: 16. 
    # If using task (32) + goal (16) = 48.
    ...
    if resume_checkpoint_path is not None:
        ...
        checkpoint = torch.load(os.path.join(resume_checkpoint_path, 'checkpoints', 'ema_model_current_state_dict.pth' if use_ema else 'model_current_state_dict.pth'), 
